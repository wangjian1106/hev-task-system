...
static int fd = -1;
static HevTaskLoggerLevel req_level;

int
hev_task_logger_init (HevTaskLoggerLevel level, const char *path)
{
    req_level = level;
...
void
hev_task_logger_fini (void)
{
    close (fd);
}

int
hev_task_logger_enabled (HevTaskLoggerLevel level)
{
...
void
hev_task_logger_log (HevTaskLoggerLevel level, const char *fmt, ...)
{
    struct iovec iov[4];
    const char *ts_fmt;
    char msg[1024];
    ...
    switch (level) {
    case HEV_TASK_LOGGER_DEBUG:
        iov[1].iov_base = "[D] ";
        break;
    case HEV_TASK_LOGGER_INFO:
        iov[1].iov_base = "[I] ";
        break;
    case HEV_TASK_LOGGER_WARN:
        iov[1].iov_base = "[W] ";
        break;
    case HEV_TASK_LOGGER_ERROR:
        iov[1].iov_base = "[E] ";
        break;
    case HEV_TASK_LOGGER_UNSET:
        iov[1].iov_base = "[?] ";
        break;
    }
    iov[1].iov_len = 4;
